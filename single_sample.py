...
def filter_ref_gene_or_transcript(input_file,output_path,gene_list=True,sequence_type='gene',exp_level='count'):
    """
    ...
    """
    if gene_list:
        list_file=open(output_path+"/ref.{}.{}.list".format(sequence_type,exp_level), 'w+')
    with open(input_file,'r+') as f1,open(output_path+"/ref.{}.{}".format(sequence_type,exp_level),'w+') as f2:
        f1.readline()
        id_list=[]
        for lines in f1:
            line=lines.strip().split("\t")
